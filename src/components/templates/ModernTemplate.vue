...
<template>
  <div class="print-area bg-white text-gray-800 max-w-[210mm] mx-auto rounded-2xl min-h-[297mm] overflow-hidden shadow-[0_8px_60px_rgba(99,102,241,0.07),0_2px_8px_rgba(0,0,0,0.03)] ring-1 ring-gray-200/40">
    <!-- Blue header banner -->
    <header class="bg-blue-600 text-white px-8 py-8 group/header">
      <div class="flex items-start gap-5">
        <!-- Avatar -->
        <div class="shrink-0 group/avatar relative">
          <div
            v-if="data.personal.avatar"
            class="w-18 h-18 rounded-full overflow-hidden ring-2 ring-white/30"
          >
          ...
          <button
            v-else
            class="btn btn-circle btn-outline btn-ghost no-print w-18 h-18 border-2 border-dashed border-white/30 hover:border-white/60 text-white/40 hover:text-white/70"
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-blue-100 text-lg mt-1">
      ...
      <VueDraggable
        ...
        :animation="200"
        class="flex flex-wrap gap-x-5 gap-y-1 mt-4 text-sm text-blue-200"
      >
        <span
          v-for="field in data.contactFields"
          :key="field.id"
          v-show="field.visible"
          class="flex items-center gap-1.5 contact-item group/contact"
        ...
        >
          ...
          <button
            class="btn btn-ghost btn-xs btn-circle no-print opacity-0 group-hover/contact:opacity-100 min-h-0 h-4 w-4 text-blue-300 hover:text-white"
      ...
      <button
        class="btn btn-ghost btn-xs no-print mt-2 opacity-0 group-hover/header:opacity-100 gap-1 text-blue-300 hover:text-white"
    ...
    <div class="p-6 space-y-0">
      ...
      >
        <div v-for="sectionKey in data.sectionOrder" :key="sectionKey" class="mb-6 resume-section group/section relative">
          ...
          <div class="flex items-center gap-2 mb-3">
            <h2 class="text-xs font-bold uppercase tracking-widest text-blue-600 border-b border-blue-200 pb-1 flex-1">
              <EditableField
          ...
          <template v-if="sectionKey === 'experiences'">
            <VueDraggable v-model="data.experiences" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
              <div v-for="(exp, expIdx) in data.experiences" :key="exp.id" class="mb-4 last:mb-0 relative pl-4 border-l-2 border-blue-100 group/item item-wrapper">
                <div class="absolute w-2.5 h-2.5 bg-blue-600 rounded-full left-[-7px] top-1"></div>
                <div class="flex-1 min-w-0">
          ...
          <template v-else-if="sectionKey === 'projects'">
            <VueDraggable v-model="data.projects" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
              <div v-for="(proj, projIdx) in data.projects" :key="proj.id" class="mb-4 last:mb-0 relative pl-4 border-l-2 border-blue-100 group/item item-wrapper">
                <div class="absolute w-2.5 h-2.5 bg-blue-600 rounded-full left-[-7px] top-1"></div>
                <div class="flex-1 min-w-0">
            ...
            <VueDraggable v-model="data.education" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
              <div v-for="(edu, eduIdx) in data.education" :key="edu.id" class="mb-3 last:mb-0 group/item relative item-wrapper">
                <div class="flex-1 min-w-0">
                  ...
                  <template v-else>
                    <div class="flex justify-between items-baseline gap-4">
                      <span class="font-semibold text-sm text-gray-800 flex-1 min-w-0">
                    ...
                    <div class="text-xs text-gray-500 -mt-0.5 flex items-baseline gap-1">
          ...
          <template v-else-if="data.customSections[sectionKey]">
            <VueDraggable v-model="data.customSections[sectionKey]" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
              <div v-for="(item, itemIdx) in data.customSections[sectionKey]" :key="item.id" class="mb-4 last:mb-0 relative pl-4 border-l-2 border-blue-100 group/item item-wrapper">
                <div class="absolute w-2.5 h-2.5 bg-blue-600 rounded-full left-[-7px] top-1"></div>
                <div class="flex-1 min-w-0">
      ...
      <!-- Add Section Area -->
      <div class="no-print mt-6 border-t border-dashed border-blue-100 pt-4">
        <div class="flex flex-wrap items-center gap-2 justify-center">
          <button
            v-for="key in getRemovedBuiltinSections()"
            :key="key"
            class="btn btn-xs btn-outline btn-ghost text-gray-400 hover:text-blue-600"
