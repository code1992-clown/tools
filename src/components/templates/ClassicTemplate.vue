...
function getItemArray(sectionKey: string): any[] {
  ...
  return map[sectionKey] || data.customSections[sectionKey] || []
}
</script>

<template>
  <div class="print-area bg-white text-gray-800 p-8 max-w-[210mm] mx-auto rounded-2xl min-h-[297mm] shadow-[0_8px_60px_rgba(99,102,241,0.07),0_2px_8px_rgba(0,0,0,0.03)] ring-1 ring-gray-200/40">
    <!-- Header: Personal Info -->
    <header class="relative pb-4 mb-6 rounded-xl p-3 -m-3 transition-all duration-200 hover:ring-1 hover:ring-indigo-200 hover:bg-indigo-50/20 group/header">
      <!-- Design button for personal section -->
      <button
        class="no-print absolute top-2 right-2 section-add-btn opacity-0 group-hover/header:opacity-50"
        @click="openDesigner('personal')"
        title="设计个人信息布局"
      >
      ...
      <template v-if="getSchema('personal').length">
        <DynamicFieldRenderer :fields="getSchema('personal')" :item="data.personal" />
      </template>
      <template v-else>
        <div class="flex items-start gap-4">
          <!-- Avatar -->
          <div class="shrink-0 group/avatar relative">
            <div
              v-if="data.personal.avatar"
              class="w-16 h-16 rounded-full overflow-hidden ring-2 ring-gray-200"
            >
            ...
            <button
              v-else
              class="btn btn-circle btn-outline btn-ghost no-print w-16 h-16 border-2 border-dashed border-base-300 hover:border-primary text-base-content/30 hover:text-primary"
        ...
        <VueDraggable
          ...
          :animation="200"
          class="flex flex-wrap gap-x-4 gap-y-1 mt-3 text-sm text-gray-500"
        >
          <span
            v-for="field in data.contactFields"
            :key="field.id"
            v-show="field.visible"
            class="flex items-center gap-1 contact-item group/contact"
            :class="{ 'print-hide-empty': !field.value }"
          >
            <span class="contact-handle cursor-grab active:cursor-grabbing no-print">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" v-html="getContactIcon(field.type)"></svg>
            </span>
            <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5 shrink-0 print-only hidden" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" v-html="getContactIcon(field.type)"></svg>
            <EditableField v-model="field.value" :placeholder="field.label" autoWidth />
            <button
              class="btn btn-ghost btn-xs btn-circle no-print opacity-0 group-hover/contact:opacity-100 min-h-0 h-4 w-4 text-error/50 hover:text-error"
        ...
        <!-- Add field button -->
        <button
          class="btn btn-ghost btn-xs no-print mt-2 opacity-0 group-hover/header:opacity-100 gap-1"
      ...
      <div class="modal-box rounded-2xl shadow-2xl border border-base-content/10 max-w-sm">
        ...
        <VueDraggable v-model="data.contactFields" handle=".contact-handle" :animation="200" ghost-class="drag-ghost" class="space-y-2 mb-4">
          ...
          >
            ...
            <input
              v-model="field.label"
              class="input input-ghost input-sm flex-1 text-base-content font-semibold"
              placeholder="字段名称"
            />
    ...
    <div v-if="data.personal.summary" class="mb-6 resume-section group/section relative">
    ...
    >
      <div v-for="sectionKey in data.sectionOrder" :key="sectionKey" class="mb-6 resume-section group/section relative">
        <!-- Section Header -->
        <div class="flex items-center gap-2 mb-3">
          <h2 class="text-sm font-bold uppercase tracking-widest text-gray-400 flex-1">
            <EditableField
          ...
          <button v-if="sectionKey !== 'skills'" class="section-add-btn no-print" @click="addItem(sectionKey)" title="添加">
        ...
        <template v-if="sectionKey === 'experiences'">
          <VueDraggable v-model="data.experiences" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
            <div v-for="(exp, expIdx) in data.experiences" :key="exp.id" class="mb-4 last:mb-0 group/item relative item-wrapper">
              <div class="flex-1 min-w-0">
                ...
                <template v-else>
                  <div class="flex justify-between items-baseline gap-4">
                    <span class="font-semibold text-gray-900 flex-1 min-w-0">
                      <EditableField v-model="exp.company" placeholder="公司名称" />
                    </span>
                    <span class="text-xs text-gray-400 shrink-0 flex items-center gap-1 whitespace-nowrap">
        ...
        <template v-else-if="sectionKey === 'projects'">
          <VueDraggable v-model="data.projects" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
            <div v-for="(proj, projIdx) in data.projects" :key="proj.id" class="mb-4 last:mb-0 group/item relative item-wrapper">
              <div class="flex-1 min-w-0">
            ...
            <div v-for="(edu, eduIdx) in data.education" :key="edu.id" class="mb-3 last:mb-0 group/item relative item-wrapper">
              <div class="flex-1 min-w-0">
                ...
                <template v-else>
                  ...
                  <div class="text-gray-600 text-sm -mt-0.5 flex items-baseline gap-1">
        ...
        <template v-else-if="data.customSections[sectionKey]">
          <VueDraggable v-model="data.customSections[sectionKey]" handle=".item-handle" :animation="200" ghost-class="drag-ghost">
            <div v-for="(item, itemIdx) in data.customSections[sectionKey]" :key="item.id" class="mb-4 last:mb-0 group/item relative item-wrapper">
              <div class="flex-1 min-w-0">
          ...
          <button v-if="!data.customSections[sectionKey]?.length" class="empty-add-btn no-print" @click="addCustomItem(sectionKey)">
            + 添加{{ getSectionName(sectionKey) }}
          </button>
        </template>
      </div>
    ...
    <div class="no-print mt-6 border-t border-dashed border-gray-200 pt-4">
      <div class="flex flex-wrap items-center gap-2 justify-center">
        <button
          ...
          :key="key"
          class="btn btn-xs btn-outline btn-ghost text-gray-400 hover:text-gray-600"
